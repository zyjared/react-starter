import { ensurePrefix } from '@kit/shared'
...
export type PaletteStyleLoader = () => Promise<unknown>

export type ThemePaletteLoaders = Partial<Record<Exclude<Palette, 'default'>, PaletteStyleLoader>>

const loadedPalettes = new Set<Palette>(['default'])
...
interface ThemeProviderProps {
  ...
  storageKey?: string
  paletteLoaders?: ThemePaletteLoaders
}
...
function readThemeStorageState({
  ...
  try {
    const parsed = JSON.parse(storedState) as Partial<ThemeStorageState>
...
export function ThemeProvider({
  ...
  useEffect(() => {
    const palette = state.palette
    if (palette === 'default' || loadedPalettes.has(palette) || !paletteLoaders)
      return

    const loader = paletteLoaders[palette]
