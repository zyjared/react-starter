...
- 应用层只能从 `@r/features/<feature>` 导入。
- 禁止跨 feature 深层导入（如 `@r/features/auth/api/...`）。
...
- PR/提交前必须满足：
  - `pnpm --filter @r/features verify` 通过
...
7. 运行 `pnpm --filter @r/features verify`
